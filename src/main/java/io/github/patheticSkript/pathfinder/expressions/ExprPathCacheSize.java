...
public class ExprPathCacheSize extends SimpleExpression<Integer> {
    ...
    public void change(Event event, Object [] delta, Changer.ChangeMode mode)  {
        ...
        if (!(delta[0] instanceof Integer)) { return; }
        try {
            ...
            if (Duration.between(lastError, Instant.now()).toSeconds() > 15) {
                ...
                PatheticSkript.getInstance().getLogger().warning("You can not add a non integer to an integer! " + e.getMessage() + "This message will apear at most once every 15 seconds!");
            }
        }
